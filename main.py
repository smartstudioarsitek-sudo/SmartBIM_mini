...
# ... (Semua imports dan setup tetap sama)

# ... (Tabs 1, 2, 3 tetap)

# ==============================================================================
# TAB 4: STRUKTUR BETON (SNI) - SAP RIGOR IMPLEMENTATION
# ==============================================================================
with tabs[3]:
    st.markdown('<p class="sub_header">Analisa Struktur Atas (SNI 2847)</p>', unsafe_allow_html=True)
    ...
    with c_s1:
        st.subheader("Input Gaya Dalam Desain (Wajib dari FEA)")
        
        q_dl = st.number_input("Beban Mati (DL) kN/m", 0.0, 50.0, 15.0)
        q_ll = st.number_input("Beban Hidup (LL) kN/m", 0.0, 50.0, 5.0)
        q_u = sni.SNI_Load_1727.kombinasi_pembebanan(q_dl, q_ll)
        st.info(f"Beban Kombinasi max (Qu): {q_u:.2f} kN/m")
        
        Mu = st.number_input("Momen Ultimate Mu (kNm) Max Perletakan/Lap. (WAJIB)", value=30.0, min_value=0.0, key='Mu_input')
        Vu = st.number_input("Gaya Geser Ultimate Vu (kN) Max. (WAJIB)", value=45.0, min_value=0.0, key='Vu_input')

        st.divider()

        # --- FITUR REDESAIN OTOMATIS ---
        if st.button("🔴 AUTO-REDESIGN (OPTIMASI GEMS)"):
            b_rec, h_rec = calc_sni.rekomendasi_dimensi_balok(Mu)
            st.session_state['geo']['b'] = b_rec
            st.session_state['geo']['h'] = h_rec
            st.success(f"✅ Auto-Redesign: Dimensi Balok diubah menjadi {b_rec}x{h_rec} mm.")
            st.experimental_rerun()
            
    with c_s2:
        st.subheader("Hasil Desain Penulangan Fleksur (Lentur)")
        
        b_mm = st.session_state['geo']['b']
        h_mm = st.session_state['geo']['h']
        ds = 40 
        d_eff = h_mm - ds
        
        # Hitung As, Phi, dan Rho aktual
        res_tulangan = calc_sni.kebutuhan_tulangan(Mu, b_mm, h_mm, ds)
        As_req = res_tulangan[0]
        phi_final = res_tulangan[1]
        rho_aktual = res_tulangan[2]
        
        if As_req > 0:
            st.metric("Luas Tulangan Perlu (As)", f"{As_req:.0f} mm2")
            
            # Tampilkan Cek Rigor (SAP/ETABS Style)
            st.markdown("---")
            col_r1, col_r2 = st.columns(2)
            col_r1.metric("Faktor Reduksi Kekuatan ($\phi$)", f"{phi_final:.3f}")
            col_r2.metric("Rasio Tulangan ($\rho$)", f"{rho_aktual*100:.2f}%")
            
            # Cek Daktilitas/Kegagalan
            if rho_aktual > calc_sni.rho_max:
                st.error("❌ GAGAL DAKTILITAS: Rho > Rho_Max. Balok akan Runtuh Rapuh!")
            elif phi_final < 0.9:
                st.warning("⚠️ DESAIN KRITIS: $\phi < 0.9$. Balok di Zona Transisi, pertimbangkan perbesar dimensi.")
            else:
                 st.success("✅ DESAIN AMAN & DAKTIL ($\phi=0.9$)")
                
            # Konversi ke Jumlah Besi
            dia = st.selectbox("Diameter Tulangan", [13, 16, 19, 22])
            A_bar = 0.25 * 3.14 * dia**2
            n_bars = np.ceil(As_req / A_bar)
            st.success(f"Rekomen Lentur: **{int(n_bars)} D{dia}**")
            
            
            # --- Cek GESER & DAKTILITAS (SENGKANG) ---
            st.write("---")
            st.subheader("Hasil Desain Geser & Daktilitas")
            
            dia_sengkang = st.selectbox("Diameter Sengkang (mm)", [8, 10, 12])
            res_geser = calc_sni.kebutuhan_sengkang(Vu, b_mm, d_eff, 0, dia_sengkang)
            s_req_mm = res_geser['s_req']
            
            if "GAGAL" in res_geser['status']:
                st.error(f"❌ {res_geser['status']} (Vu terlalu besar).")
            else:
                 st.metric("Jarak Sengkang Perlu", f"{s_req_mm:.0f} mm")
                 st.info(f"Vs perlu: {res_geser['Vs_need']:.2f} kN")
                 
            # Cek Detailing Daktilitas (Zona Kritis)
            res_daktil = calc_sni.cek_daktilitas_balok(b_mm, h_mm, st.session_state['geo']['L'], dia_sengkang, As_req)
            st.warning(f"ZONA KRITIS: Sengkang harus dipasang $\\leq$ **{res_daktil['s_max_kritis']} mm** sepanjang **{res_daktil['Panjang_Kritis']:.0f} mm** di ujung balok.")
            
        else:
            st.error("❌ Kegagalan Desain: Momen terlalu besar (As overflow).")
            # ... (Tampilkan Rekomendasi GEMS)
            b_rec, h_rec = calc_sni.rekomendasi_dimensi_balok(Mu)
            st.error(f"**REKOMENDASI GEMS:** Perbesar dimensi balok menjadi **{b_rec}x{h_rec} mm**")
            n_bars = 0
            
        # ... (Logika penyimpanan QTO tetap sama)
